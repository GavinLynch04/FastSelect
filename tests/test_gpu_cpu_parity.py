...
def test_relieff_gpu_neighbor_buffers_match_cpu():
    ...
    cpu = ReliefF(n_features_to_select=8, n_neighbors=10, backend="cpu").fit(X, y)
    gpu = ReliefF(n_features_to_select=8, n_neighbors=10, backend="gpu").fit(X, y)
...
def test_relieff_gpu_large_neighbor_fallback_matches_cpu():
    ...
    cpu = ReliefF(n_features_to_select=4, n_neighbors=4, backend="cpu").fit(X, y)
    gpu = ReliefF(n_features_to_select=4, n_neighbors=4, backend="gpu").fit(X, y)
...
def test_gpu_score_accumulator_paths_agree(estimator_cls, use_star):
    """Both GPU scoring kernels must produce the same scores as the CPU kernel.

    SURF and MultiSURF pick between a shared-memory accumulator and a
    global-atomic accumulator based on the feature count, so this covers a
    dataset on each side of ``SHARED_SCORE_MAX_FEATURES``.
    """
    from fast_select.SURF import SHARED_SCORE_MAX_FEATURES

    rng = np.random.default_rng(4242)
    for n_features in (SHARED_SCORE_MAX_FEATURES, SHARED_SCORE_MAX_FEATURES + 64):
        X = rng.standard_normal((60, n_features), dtype=np.float32)
        X[:, -4:] = rng.integers(0, 3, size=(60, 4))
        y = np.tile(np.arange(2, dtype=np.int32), 30)
        ...
        cpu = estimator_cls(backend="cpu", **kwargs).fit(X, y)
        gpu = estimator_cls(backend="gpu", **kwargs).fit(X, y)
        ...
        assert_allclose(
            ...
            rtol=1e-4,
            atol=1e-6,
            err_msg=(f"{estimator_cls.__name__} star={use_star} disagreed at " f"n_features={n_features}"),
        )

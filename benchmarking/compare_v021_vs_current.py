"""Sustained benchmark of published v0.2.1 against the working tree.

The parent process extracts the published v0.2.1 commit and starts a fresh
Python process for every version/case pair. JIT compilation is warmed up and
excluded. Each reported timing contains at least ``--min-seconds`` seconds of
measured fits.
"""
...
def _extract_v021(repo_root: Path, destination: Path) -> None:
    archive_path = destination.parent / "fast-select-v021.zip"
...
def _parent(args: argparse.Namespace) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="fast-select-v021-") as temp:
        baseline_root = Path(temp) / "source"
        baseline_root.mkdir()
        _extract_v021(repo_root, baseline_root)
        ...
        for estimator, backend in cases:
            for round_index in range(args.rounds):
                ...
                variants = [
                    ("published-v0.2.1", baseline_root),
                ...
                speedup = (
                    round_results["published-v0.2.1"]["mean_seconds"] / round_results["working-tree"]["mean_seconds"]
                )
                print(f"    round speedup {speedup:.3f}x", flush=True)

    print("\nSummary (lower runtime is better)")
    print("estimator   backend  v0.2.1(s)  current(s)  speedup  traced-memory change")
    for estimator, backend in cases:
        matching = [item for item in results if item["estimator"] == estimator and item["backend"] == backend]
        baseline = [item for item in matching if item["version"] == "published-v0.2.1"]
        ...
        print(
            f"{estimator:<11} {backend:<7} {baseline_time:>10.6f} "
            f"{current_time:>11.6f} {speedup:>8.3f}x "
            f"{memory_delta:>+10.2f} MiB"
        )
